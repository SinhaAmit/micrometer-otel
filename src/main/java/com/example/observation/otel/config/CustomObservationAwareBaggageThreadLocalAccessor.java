 ...
 * @since 1.2.2
 */
public class CustomObservationAwareBaggageThreadLocalAccessor implements ThreadLocalAccessor<BaggageToPropagate> {

    private static final InternalLogger log = InternalLoggerFactory
        .getInstance(CustomObservationAwareBaggageThreadLocalAccessor.class);

    final ThreadLocal<BaggageAndScope> baggageInScope = new ThreadLocal<>(); //Replaced concurrentHashMap with ThreadLocal
     ...
     * @since 1.2.2
     */
    public CustomObservationAwareBaggageThreadLocalAccessor(ObservationRegistry observationRegistry, Tracer tracer) {
    ...
    private BaggageAndScope scopeRestoringBaggageAndScope(BaggageAndScope currentScope, BaggageAndScope previousScope) {
        return currentScope.andThen(o -> {
            if (previousScope != null) {
                ...
                if (log.isTraceEnabled()) {
                    log.trace("No previous scope to put, clearing the thread [" + "" + "]");
                }
